...
def gcode_overlay(img_size_x, img_size_y, gcode_file, image_and_layer, LAYER_NUMBER, im_projection):
    ...
    ot_x = -10.0 # move gcode (-)up / (+)down
    ot_y = 1.75  # move gcode left/right
    ot_z = -50
    ...
    fig = plt.figure(figsize=(img_size_x*px,img_size_y*px),dpi=100,frameon=False)
    plt.imshow(img)

    #plt.xlabel('X, mm')
    #plt.ylabel('Y, mm')
    plt.axis('off')
    plt.margins(0,0,tight='tight')
    ...
    plt.savefig(im_projection+'.jpg',dpi=100,transparent = True, bbox_inches = 'tight', pad_inches = 0)
    im = cv2.imread(im_projection+'.jpg')
    resize_img = cv2.resize(im, (img_size_x,img_size_y))
    resize_img.shape
    cv2.imwrite(im_projection+'.jpg',resize_img)
    
    plt0 = plt
    
    def drawGrid(plt, im_projection,number_lines):
    ####################### Add grid
        number_lines = 24
        spacingx = int(img_size_x / number_lines) # spacing in pixels
        spacingy = int(img_size_y / number_lines) # spacing in pixels
        gridlinesv=np.zeros([number_lines+2,2])
        for i in range(np.shape(gridlinesv)[0]): # Vertical Lines
            if i%2==0:
                # Start coordinate top left corner
                gridlinesv[i] = [spacingx*i, 0]   # x, y
                # End coordinate bottom right corner of image
                gridlinesv[i+1] = [spacingx*i, img_size_y]
        gridlinesh=np.zeros([number_lines+2,2])
        for i in range(np.shape(gridlinesh)[0]): # Hor. Lines
            if i%2==0:
                # Start coordinate top left corner
                gridlinesh[i] = [0,spacingy*i]   # x, y
                # End coordinate bottom right corner of image
                gridlinesh[i+1] = [img_size_x,spacingy*i]

        # Line thickness
        thickness = 1
        for i in range(np.shape(gridlinesv)[0]-1):
            if i%2==0:
                    plt.plot([gridlinesv[i+1][0],gridlinesv[i][0]],[gridlinesv[i+1][1],gridlinesv[i][1]],\
                     color='g',linewidth=thickness)
                    plt.plot([gridlinesh[i+1][0],gridlinesh[i][0]],[gridlinesh[i+1][1],gridlinesh[i][1]],\
                     color='g',linewidth=thickness)

        plt.savefig('grid_'+im_projection+'.jpg',dpi=100,transparent = True, bbox_inches = 'tight', pad_inches = 0)
        im = cv2.imread('grid_'+im_projection+'.jpg')
        resize_img = cv2.resize(im, (img_size_x,img_size_y))
        resize_img.shape
        cv2.imwrite('grid_'+im_projection+'.jpg',resize_img)
    
    def blackout_contour(plt, image, outer_outline, im_projection):
        image = cv2.imread(image)
        simplified = simplify_coords(outer_outline, 1.0)
        simplified = np.round(simplified,2)
        stencil = np.zeros(image.shape).astype(image.dtype)
        color = [255, 255, 255]
        cv2.fillPoly(stencil, np.int32([simplified[0:-6]]), color)
        result = cv2.bitwise_and(image, stencil)
        cv2.imwrite('blackout_'+im_projection+'.jpg', result)
        
    def split(image, window_size, margin):
    ...
    #drawGrid(plt0, im_projection,24)
    blackout_contour(plt, image_and_layer, tGp_wall_outer, im_projection)

    out = split('blackout_'+im_projection+'.jpg',window_size=384,margin=16)
    dimsplit = np.shape(out)
    
    n = 0
    for i in range(dimsplit[0]):
        cv2.imwrite('split'+str(i)+'_'+im_projection+'.jpg',out[i])
        image = cv2.imread('split'+str(i)+'_'+im_projection+'.jpg',0)
        if cv2.countNonZero(image) == 0:
            #image all black, remove these
            os.remove('split'+str(i)+'_'+im_projection+'.jpg')
        else:
            n+=1 # number of images saved
    print(n)       
    print('\nProjection images saved.\n')
    return tGp_wall_outer