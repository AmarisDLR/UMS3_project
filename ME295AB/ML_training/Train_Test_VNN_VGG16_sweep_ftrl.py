...
base_dir = '../UMS3_dd_Mar31/training_randomized'
...
x = Flatten()(last)
x = Dense(config.dense_units, activation='relu', name='fc1')(x) #800 #relu
x = Dropout(config.dropout)(x) #0.3
...
'''lr = schedules.ExponentialDecay(initial_learning_rate=1e-5, decay_steps=5000,decay_rate=0.99)'''
#optimizer=Adam(learning_rate=config.learn_rate, epsilon=config.eps)
#optimizer=Adamax(learning_rate=lr, beta_1=0.95, beta_2=0.97, epsilon=1e-05)
#optimizer=Nadam(learning_rate=lr, beta_1=0.85, beta_2=0.97, epsilon=1e-04)
#optimizer=Adagrad(learning_rate=config.learn_rate,
   # initial_accumulator_value=0.6, epsilon=config.eps)
optimizer=Ftrl(learning_rate=config.learn_rate, 
    learning_rate_power=config.learn_rate_power,
    initial_accumulator_value=config.iac,
    l1_regularization_strength=config.L1,
    l2_regularization_strength=config.L2,
    l2_shrinkage_regularization_strength=config.L2_srink,
    beta=config.beta)
...
print(f'Learning rate: {config.learn_rate}, epochs: {epochs}, batch size: {batch_size}, dropout: {config.dropout}, optimizer: {optimizer}')
wandb.config = {
